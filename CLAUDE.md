...
- Both arms write to one `results-<cli>.tsv` in the examples repo (`case / arm / cli / model / cli_rc / score / build_s / tool_calls / run_at`); the `arm` column separates them, so comparing is `column -t < results-claude.tsv`. Split per CLI because comparing a claude skill run against a codex baseline measures the CLI, not the skill.
