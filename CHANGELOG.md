...
- `run-baseline.sh` produced a control arm that wasn't controlled. It wrote into `seedkit-examples/baselines/`, below the `.claude/skills/seedkit` symlink `run-tests.sh` creates, while telling the agent "there is no skill loaded" — and for `agy`, the skill is installed globally. Output moves to a sibling `seedkit-baselines/`, and `assert_skill_unreachable()` walks up from the baseline root and refuses to start if the skill is reachable or globally installed.
...
- Both scripts append a row to `results-<cli>.tsv` (`case / arm / cli / model / boot_rc / score / duration_s / tool_calls`), so comparing the arms is a table rather than eyeballing two trees. Split per CLI so a claude skill run can't be compared against a codex baseline — that measures the CLI, not the skill. `agents.sh` emits `[tool:NAME]` markers for claude (codex already did) and `count_tool_calls` totals them — reaching a working project in fewer loops is an outcome the skill gets credit for.
- Baseline logs move out of `seedkit-examples/logs/`, which `review-logs.sh` globs — it was picking up `baseline-*.log` files and trying to review them as testcase runs.
