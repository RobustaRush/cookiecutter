...
- Both arms write one `results-<cli>.tsv` in the examples repo (`case / arm / cli / model / boot_rc / score / duration_s / tool_calls / run_at`), so comparing them is `column -t` rather than eyeballing two trees. Split per CLI so a claude skill run can't be compared against a codex baseline — that measures the CLI, not the skill. `upsert_result()` replaces the row for a `(case, arm)` pair, so re-running a case after a skill fix updates its result instead of appending a second row beside the stale one.
